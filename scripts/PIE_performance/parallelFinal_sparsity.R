...
for (i in c(1:k)){
  ...
  dim(fold_summy)
    cv_matrix[i,] = fold_summy[which(fold_summy$rrmse_validation_test==min(fold_summy[which(fold_summy$rrmse_Validation_GAM_only < 1 & fold_summy$sparsity<=8 ),]$rrmse_validation_test))[1],]
}
