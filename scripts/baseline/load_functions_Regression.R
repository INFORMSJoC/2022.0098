...
DecisonTree_CV<-function(formula,data,nfold){
  ...
  names(fold_errorMat)<-c("method_name","fold_num","rrmse_train","rrmse_test", "mae_train","mae_test","parameter","parameter range")
  fold_num = 0
  for(fold in 1:nfold){
    #data split####
    #cross validation
...
DecisionTree<-function(formula,data,train_X_orig,train_y,test_X_orig,test_y,nfold){
  ...
  MAE_train = mae(train_predict,train_y)
  ##Test####
  ...
  MAE_test = mae(test_predict,test_y)
  
  errorMat<-c(rrMSE_train,rrMSE_test,MAE_train,MAE_test)

  errorMat_result<-list("rrmse_train"=errorMat[1],"rrmse_test"=errorMat[2],
                        "mae_train"=errorMat[3],"mae_test"=errorMat[4],
...
Lasso_or_Ridge<-function(train_X_orig,train_y,alpha,test_X_orig,test_y,nfold){
  ...
  print(best_lam)
  lasso_or_ridge_best <- glmnet(data.matrix(train_X_orig),train_y, type.measure='mae',alpha = alpha, lambda = best_lam)
...
Gam<-function(formula,data,nfold,train_X_orig,train_y,test_X_orig,test_y){
  ...
  gam_fit <- train(formula,data = data, method="gamSpline",metric="MAE",df = 5, trControl = trControl) #with default, no parameter tuned
  #train
...
XgBoost<-function(dtrain,train_y,dtest,test_y,params,nrounds){
  set.seed(100)
  xgb_fit<-xgb.train (params = params, data = dtrain, nrounds =nrounds, eval_metric = "mae")
  #train
  ...
  print("test working")
  errorMat<-c(rrMSE_train,rrMSE_test,MAE_train,MAE_test)
  names(errorMat) = c("rrmse_train","rrmse_test", "mae_train","mae_test")
  errorMat
}
XgBoost_summary<-function(train_X_orig,train_y, test_X_orig,test_y,params,nrounds_max, nfold){
  ...
  errorMat_result<-list("rrmse_train"=Xg_error[1],"rrmse_test"=Xg_error[2],
                        "mae_train"=Xg_error[3],"mae_test"=Xg_error[4],
